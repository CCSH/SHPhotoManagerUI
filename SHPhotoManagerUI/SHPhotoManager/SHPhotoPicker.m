...
@interface SHPhotoPicker ()<UICollectionViewDataSource,
...
- (void)configUI {
    ...
    //添加点击
    [self.bgView addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(dismissPhotoPicker)]];
    [self addSubview:self.bgView];
    
    //下方展示视图
    self.photoSheet = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_H - picker_H, SCREEN_W, picker_H)];
    ...
    self.collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_W, picker_H - 30) collectionViewLayout:layout];
    ...
    //下方按钮
    //完成
    self.doneBtn = [UIButton buttonWithFrame:CGRectMake(SCREEN_W - 80, picker_H - 30, 60, 24) title:@"完成" target:self selector:@selector(endPick)];
    ...
    CGFloat View_X = 20;
    //相机
    UIButton * cameraBtn = [UIButton buttonWithFrame:CGRectMake(View_X, picker_H - 30, 40, 20) title:@"相机" target:self selector:@selector(takePhoto)];
    [self.photoSheet addSubview:cameraBtn];
    View_X += 60;
    
    //相册
    UIButton * albumBtn = [UIButton buttonWithFrame:CGRectMake(View_X, picker_H - 30, 40, 20) title:@"相册" target:self selector:@selector(albumBrowser)];
    [self.photoSheet addSubview:albumBtn];
    View_X += 60;
    
    //按钮图片
    self.originalBtn = [UIButton buttonWithFrame:CGRectMake(View_X, picker_H - 30, 18, 18) Target:self Selector:@selector(originalSwitch:) Image:@"photo_unselected.png" ImageSelected:@"photo_selected.png"];
    [self.photoSheet addSubview:self.originalBtn];
    View_X += 18;
    
    //原图
    UIButton * originalTextBtn = [UIButton buttonWithFrame:CGRectMake(View_X, picker_H - 30, 40, 20) title:@"原图" target:self selector:@selector(originalSwitch:)];
    [self.photoSheet addSubview:originalTextBtn];
}

- (void)refreshDoneStatus {
    if ([SHPhotoCenter shareCenter].selectedPhotos.count > 0) {
        [self.doneBtn setTitle:[NSString stringWithFormat:@"完成(%ld)",(unsigned long)[SHPhotoCenter shareCenter].selectedPhotos.count] forState:UIControlStateNormal];
...
- (void)showInSender:(UIViewController *)sender handle:(void(^)(NSArray<UIImage *> * photos))handle {
    ...
    [SHPhotoCenter shareCenter].handle = handle;
...
- (void)fetchImageFromAsset:(PHAsset *)asset completeBlock:(void(^)(UIImage * image, NSDictionary * info))completeBlock {
    CGSize size = [self calculateImgSizeFromAsset:asset];
    [[SHPhotoManager manager]fetchImageInAsset:asset size:size isResize:YES completeBlock:^(UIImage *image, NSDictionary *info) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    [[SHPhotoCenter shareCenter] endPickWithImage:image];
