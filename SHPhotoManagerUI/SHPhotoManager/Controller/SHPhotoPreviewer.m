...
- (void)refreshCompleteBtnStatus {
    if ([SHPhotoCenter shareCenter].selectedPhotos.count) {
        self.sendCount.text = [NSString stringWithFormat:@"完成(%ld)",[SHPhotoCenter shareCenter].selectedPhotos.count];
    }else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[SHPhotoManager manager]fetchImageInAsset:self.dataSource[indexPath.row] size:CGSizeMake(cell.w, cell.h) isResize:NO completeBlock:^(UIImage *image, NSDictionary *info) {
