...
- (void)setupUI {
    ...
    self.collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_W, SCREEN_H - 64 - 38) collectionViewLayout:layout];
...
- (void)refreshBottomView {
    if ([SHPhotoCenter shareCenter].selectedPhotos.count > 0) {
        ...
        self.comBtn.text = [NSString stringWithFormat:@"完成(%d)",(int)[SHPhotoCenter shareCenter].selectedPhotos.count];
    }else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[SHPhotoManager manager]fetchImageInAsset:self.dataSource[indexPath.row] size:CGSizeMake(cell.w * 2, cell.h * 2) isResize:YES completeBlock:^(UIImage *image, NSDictionary *info) {
